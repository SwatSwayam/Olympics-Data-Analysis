...
df_summer = preprocessor.preprocess()
...
if user_menu == 'Overall Analysis':
    ...
    nations_overtime = helper.data_overtime(df_summer, 'region')
    fig = px.line(nations_overtime, x='Edition', y='region')
    st.title("Participating Nations over the years:")
    st.plotly_chart(fig)

    events_overtime = helper.data_overtime(df_summer, 'Event')
    fig = px.line(events_overtime, x='Edition', y='Event')
    st.title("Events over the years:")
    st.plotly_chart(fig)

    athletes_overtime = helper.data_overtime(df_summer, 'Name')
    ...
    x = df_summer.drop_duplicates(['Year', 'Sport', 'Event'])
    sns.heatmap(x.pivot_table(index='Sport', columns='Year', values='Event', aggfunc='count').fillna(0).astype('int'), annot= True)
    st.pyplot(fig)

    st.title("Most Successful Athletes:")
    sports_list = df_summer['Sport'].unique().tolist()
    sports_list.sort()
    sports_list.insert(0, 'Overall')
    selected_sport = st.selectbox("Select a sport", sports_list)
    x = helper.most_successful(df_summer, selected_sport)
    st.table(x)
...
if user_menu == 'Country-wise Analysis':
    ...
    top10_df = helper.most_successful_country_wise(df_summer,selected_country)
    st.table(top10_df)
...
if user_menu == 'Athlete-wise Analysis':
    ...
    temp_df = helper.weight_v_height(df_summer, selected_sport)
    fig, ax = plt.subplots()
    ...
    final = helper.men_vs_female(df_summer)
